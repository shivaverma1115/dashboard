...
const CancelOrderTable = () => {
  const { header, user, updateStatus, setUpdateStatus } = useGlobalContext();
  ...
  const handleInputChange = (e: any) => {
    ...
    axios
      .get(
        `${process.env.BASE_URL}success/search-cancel-orders-pending?search=${searchValue}`
      )
      .then((res) => {
  ...
  const handleUpdateStstus = (item: CanceOrderDataType) => {
    ...
    axios
      .put(
        `${process.env.BASE_URL}success/accept-cancel-order?email=${user?.email}`,
        acceptCancelOrder,
        header
      )
      .then((res) => {
      ...
      <div className="cashier-content-area mt-[30px] px-7">
        <div className="cashier-salereturns-area bg-white p-7 custom-shadow rounded-lg pt-5 mb-5">
          ...
          {cancelOrderData?.length ? (
                    ...
                    {cancelOrderData.length ? (
                        ...
                        >
                          ...
                          <div className="cashier-salereturns-table-companyF">
                            <div className="dropdown">
                              <button
                                onClick={() => handleUpdateStstus(item)}
                      ...
                      <>
                        {loading ? (
                          ...
                          <>
                            <p className="text-center"> See Cancled Orders </p>
                          </>
                        )}
                      </>
                    )}
            ...
            <>
              <p className="text-center mt-5"> No Cancel Order Found </p>
            </>
          )}
        </div>
      </div>
